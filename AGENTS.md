...
`blauhoss1982-code/PECTEST` is the durable source of truth for PECTEST project state, task contracts, execution evidence, and lifecycle decisions. Planner chat is working context only. The PEC repository and C1 register are transport only and never replace PECTEST project authority.

## Roles

- Planner / Owner-side coordinator owns project interpretation, PECTEST Issues, planning branches and PRs, versioned active-task contracts, exact execution binding, independent review, `ACCEPT | REWORK | REPLAN`, execution PR/merge, closeout, and next-step selection.
...
2. After task merge, Planner creates the dedicated execution branch at the merged Start and durably records the exact Repository / Branch / Start / Task / Task-blob binding. The Task-blob freezes the executable contract.
3. Executor must use only those durable PECTEST coordinates. Chat text, PEC transport metadata, another branch, or another task revision cannot substitute for them.
...
PEC C1 carries only explicit `CONTINUE | TERMINAL` transport dispositions. PEC authority identifiers belong to the transport repository and are not PECTEST task authority. Detailed implementation instructions stay in the merged PECTEST active task.
