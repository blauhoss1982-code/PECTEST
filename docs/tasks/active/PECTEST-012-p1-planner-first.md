# PECTEST-012 — P1 planner-first counted workload
...
- Durable task path: `docs/tasks/active/PECTEST-012-p1-planner-first.md`
- PECTEST GitHub is the durable project authority.
- PEC C2 is transport only. PEC metadata or chat text never substitutes for this merged task and its exact execution binding.

## Objective

Complete the P1 Planner-first counted workload by creating repository-root `e2e-planner-first.txt` with exact bytes:

`b"PECTEST_PLANNER_FIRST_E2E_OK\n"`
...
1. Create repository-root `e2e-planner-first.txt` with exact bytes `b"PECTEST_PLANNER_FIRST_E2E_OK\n"`.
2. Create `tests/test_e2e_planner_first.py` using only the Python standard library. It must assert the exact bytes of repository-root `e2e-planner-first.txt`.
3. Create `docs/reports/PECTEST-012-p1-planner-first-validation.md` recording:
   - Task ID `PECTEST-012`;
...
- `e2e-planner-first.txt` has exact bytes `b"PECTEST_PLANNER_FIRST_E2E_OK\n"`;
- `tests/test_e2e_planner_first.py` uses only Python standard library and asserts exact bytes;
- `docs/reports/PECTEST-012-p1-planner-first-validation.md` exists and truthfully records required evidence;
...
Executor PASS is evidence only and never constitutes Planner acceptance. Ordinary implementation defects are REWORK on the same frozen task and execution branch. Contract changes require REPLAN through a new reviewed planning change before further execution.
...
3. move this active task unchanged to `docs/tasks/completed/PECTEST-012-p1-planner-first.md` so the completed blob remains identical to the frozen Task-blob;
4. remove the active task path;
5. update `docs/CURRENT_STATUS.md` with accepted execution and closeout evidence and P1 terminal state;
6. merge the closeout PR;
7. close PECTEST Issue #75;
