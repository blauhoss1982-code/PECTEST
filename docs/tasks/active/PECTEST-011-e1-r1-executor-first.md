# PECTEST-011 — E1-R1 executor-first counted workload
...
- Durable task path: `docs/tasks/active/PECTEST-011-e1-r1-executor-first.md`
...
Complete the fresh E1-R1 Executor-first counted workload by creating repository-root `e2e-executor-first-rerun-1.txt` with exact bytes:

`b"PECTEST_EXECUTOR_FIRST_E2E_RERUN_1_OK\n"`
...
1. Create repository-root `e2e-executor-first-rerun-1.txt` with exact bytes `b"PECTEST_EXECUTOR_FIRST_E2E_RERUN_1_OK\n"`.
2. Create `tests/test_e2e_executor_first_rerun_1.py` using only the Python standard library. It must assert the exact bytes of repository-root `e2e-executor-first-rerun-1.txt`.
3. Create `docs/reports/PECTEST-011-e1-r1-executor-first-validation.md` recording:
   - Task ID `PECTEST-011`;
...
- `e2e-executor-first-rerun-1.txt` has exact bytes `b"PECTEST_EXECUTOR_FIRST_E2E_RERUN_1_OK\n"`;
- `tests/test_e2e_executor_first_rerun_1.py` uses only Python standard library and asserts exact bytes;
- `docs/reports/PECTEST-011-e1-r1-executor-first-validation.md` exists and truthfully records required evidence;
...
3. move this active task unchanged to `docs/tasks/completed/PECTEST-011-e1-r1-executor-first.md` so the completed blob remains identical to the frozen Task-blob;
4. remove the active task path;
5. update `docs/CURRENT_STATUS.md` with accepted execution and closeout evidence and E1-R1 terminal state;
6. merge the closeout PR;
7. close PECTEST Issue #69;
