# PECTEST-007 — Two-checkpoint PEC Collaboration Validation
...
Execution may begin only after this task is merged to `main`, a dedicated execution branch is created at the exact merged Start, and Planner durably records the exact binding:
...
1. Create repository-root `pec-validation.txt` as UTF-8 without BOM using LF newlines.
2. Its exact bytes after Checkpoint A must be `PEC_VALIDATION_FIRST_OK\n`.
3. Do not add the second validation line yet.
4. Do not create the Checkpoint B test or report yet.
...
Planner independently checks the remote execution branch, commit/diff, and exact `pec-validation.txt` bytes. If implementation is defective but this contract remains correct, Planner chooses `REWORK` on the same task/branch. If accepted, Planner sends a new PEC `CONTINUE` for the same exact binding; only that later `CONTINUE` authorizes Checkpoint B.
...
2. Final `pec-validation.txt` exact bytes must be `PEC_VALIDATION_FIRST_OK\nPEC_VALIDATION_SECOND_OK\n`.
...
4. Add `docs/reports/PECTEST-007-validation-report.md` containing:
   - Task ID and checkpoint `B`;
   - the bound Repository / Branch / Start / Task / Task-blob coordinates from durable PECTEST state;
   - a statement that Checkpoint A's first line was preserved unchanged;
...
Both commands must pass. Then commit and push the same bound execution branch and return a compact receipt containing Task ID, checkpoint `B`, branch, bound Start, resulting HEAD/commit, both test results, report path, and push result. Stop for Planner review.
...
1. Planner opens and merges a normal execution PR into `main`;
2. Planner creates a separate closeout planning branch/PR;
3. closeout moves this active task **unchanged** from `docs/tasks/active/PECTEST-007-pec-validation.md` to `docs/tasks/completed/PECTEST-007-pec-validation.md`;
4. closeout updates `docs/CURRENT_STATUS.md` with completion evidence;
5. Planner merges the closeout PR;
6. Planner closes PECTEST Issue #37;
7. only then may Planner send PEC `TERMINAL`.
